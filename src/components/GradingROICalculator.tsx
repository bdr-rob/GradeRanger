...
export default function GradingROICalculator({ costBasis: initialCostBasis, gradedValues, aiGrade }: Props) {
  ...
  GRADING_SERVICES.forEach((svc) => {
    ...
    const grade = aiGradeTier && svcValues[aiGradeTier] != null ? aiGradeTier : Object.keys(svcValues).sort().pop();
    ...
    <div className="space-y-5">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {rows.map((row) => (
            <ROIRow key={row.service} {...row} costBasis={costBasis} />
          ))}
        </div>
      )}
    </div>
